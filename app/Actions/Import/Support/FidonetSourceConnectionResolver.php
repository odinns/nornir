...
class FidonetSourceConnectionResolver
{
    /**
    ...
    public function resolveConfig(string $envPath): array
    {
        if (! File::exists($envPath) || ! File::isFile($envPath)) {
            throw new InvalidArgumentException('FidoNet source env file was not found.');
        }
        ...
        if (! is_string($driver) || $driver === '') {
            throw new InvalidArgumentException('FidoNet source env file is missing DB_CONNECTION.');
        }
        ...
        if ($driver === 'sqlite') {
            ...
            if (! is_string($database) || $database === '') {
                throw new InvalidArgumentException('FidoNet source env file is missing DB_DATABASE.');
            }
        ...
        throw new InvalidArgumentException("Unsupported FidoNet source driver [{$driver}].");
    }

    public function connect(string $envPath): ConnectionInterface
    {
        $connectionName = 'fidonet_source_'.sha1($envPath);
